...
public class HomeController implements Initializable {
    ...
    @Override
    /**
     * Initializes the HomeController by setting up the UI with the details of the currently reading book,
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        rec.setArcWidth(20);
        rec.setArcHeight(20);
        ImagePattern pattern = new ImagePattern(new Image(readingBook.getImage_path()));
        rec.setFill(pattern);
        ...
        // Add a listener to update the highest rated books when the genre selection changes
    ...
    /**
     * Adds a book to the highest-rated books display by creating a new card for the book 
     * and adding it to the UI. It also sets up event listeners for showing book details when 
    ...
    private void addBookToHighestRatedBooks(Book book) {
        try {
            ...
            // Set event listener to show the clicked book's details when its card is clicked
    ...
    /**
     * Adds a hover effect to the given reading book element. When the mouse enters the book's area,
     * the book will scale up slightly to give a visual cue. When the mouse exits, the book will scale back to its original size.
     * Additionally, the cursor changes to a hand when hovering over the book and resets when the mouse leaves.
     *
     * @param readingBook the HBox containing the reading book element to apply the hover effect.
     */
    private void addReadingBookEffect(HBox readingBook) {
        ...
        // Set mouse enter event to trigger the scale-up effect and change the cursor to a hand
        readingBook.setOnMouseEntered(event -> {
            scaleUp.playFromStart();
            readingBook.setStyle("-fx-cursor: hand;");
        });

        // Set mouse exit event to trigger the scale-down effect and reset the cursor to default
    ...
    /**
     * Adds a hover effect to the book card, where the card scales up when the mouse enters
     * and scales back down when the mouse exits. Additionally, the rating bar becomes visible
     * when hovering over the card and is hidden when the mouse leaves the card.
     *
     * @param cardBox the VBox containing the book card to apply the hover effect.
     * @param cardController the controller for the book card, used to manage the rating bar visibility.
     */
    ...
    /**
     * Sets the given book as the selected book in the model. This method is called when a user
    ...
    /**
     * Updates the list of highest-rated books based on the selected genre. This method clears
     * the current list of books and populates it with the books that match the selected genre.
     *
