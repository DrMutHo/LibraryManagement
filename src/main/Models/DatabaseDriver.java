...
public class DatabaseDriver {
    ...
    public ResultSet getAllTransaction() {
        ResultSet resultSet = null;
        String query = "SELECT * FROM BorrowTransaction";
    ...
    public ResultSet getAllBorrowersForBook(int bookId) {
        ResultSet resultSet = null;
        String query = "SELECT " +
                       "c.client_id, " +
                       "c.username, " +
        ...
        try (Connection connection = this.dataSource.getConnection();
            ...
            preparedStatement.setInt(1, bookId);  // Set the bookId parameter
    ...
    public int AdminBookNoti(int lastBook){
        String query2 = "SELECT book_id from Book where ID >= ? LIMIT 1";
        try {
            Connection conn = getConnection();
            PreparedStatement stmt2 = conn.prepareStatement(query2);
            stmt2.setInt(1, lastBook);  // Pass the last known user ID
            ResultSet rs2 = stmt2.executeQuery();
            if (rs2.next()) {
              return rs2.getInt("book_id");
            }
            else return lastBook;
        } catch (Exception e) {
            e.printStackTrace();
        }
        return -1;
    ...
    public int AdminClientNoti(int lastClient){
        String query2 = "SELECT client_id from Book where ID >= ? LIMIT 1";
        try {
            Connection conn = getConnection();
            PreparedStatement stmt2 = conn.prepareStatement(query2);
            stmt2.setInt(1, lastClient);  // Pass the last known user ID
            ResultSet rs2 = stmt2.executeQuery();
            if (rs2.next()) {
              return rs2.getInt("client_id");
            }
            else return lastClient;
        } catch (Exception e) {
            e.printStackTrace();
        }
        return -1;
    ...
    public int AdminTransactionNoti(int lastTransactionID){
        String query2 = "SELECT transaction_id from Book where ID >= ? LIMIT 1";
        try {
            Connection conn = getConnection();
            PreparedStatement stmt2 = conn.prepareStatement(query2);
            stmt2.setInt(1, lastTransactionID);  // Pass the last known user ID
            ResultSet rs2 = stmt2.executeQuery();
            if (rs2.next()) {
              return rs2.getInt("transaction_id");
            }
            else return lastTransactionID;
        } catch (Exception e) {
            e.printStackTrace();
        }
        return -1;
